gbm.valuemap <- function(dbase,  # data.frame to load. Expects Lon, Lat & data columns: predicted abundances, fishing effort etc. E.g.: Abundance_Preds_All.csv from gbm.auto
                       ...
                       ...) {  # optional terms for gbm.map: byx byy mapmain heatcol shape mapback landcol legendtitle lejback legendloc grdfun zero quantile species
# Check & load gbm.map
...
for (j in 1:length(goodcols)) {  # j loop through gooddata columns
  ...
  ####HRMSY limit map####
if ("close" %in% plotthis) {  # min area for HRMSY, starting with best fish cells
   ...
   for (k in 1:nrow(dbase)) { # run the loop for every row, if required
      print(paste("Run ",((o - 1) * length(maploopcodes)) + j," of ", length(goodcols) * length(maploopcodes),"; ",n,", ",round((sum(dbase[n:nrow(dbase),goodcols[j]])/CPUEMSY) * 100, 3),"%",sep = ""))  # progress printer
          if (sum(dbase[n:nrow(dbase),goodcols[j]]) < CPUEMSY) {n = n - 1} else { #if sum of rows form end to this point < CPUEMSY aka HRMSY% add another row and sum again, ELSE:
            ...
            # this method counts UP from the WORST row (last) until reaching HRMSY% (e.g. 8%) then closes the inverse
