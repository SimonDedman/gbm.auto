gbm.auto <- function(grids = NULL,     # explantory data to predict to. Import with (e.g.) read.csv and specify object name. Defaults to NULL (won't predict to grids)
            ...
            #tc = c(2,5),              # list of permutations of tree complexity allowed, defaults to c(2,length(expvar)) @L55
            ...
            cols = grey.colors(1,1,1), # barplot colour vector. Assignment in order of explanatory variables. Default: 1*white i.e. blank bars (has border). 1*white because it repeats.
            ...
            ...)                       # additional/optional parameters, for tc & gbm.map (byx, byy, mapmain, heatcolours, colournumber, shape, mapback, landcol, legendtitle, lejback, legendloc, grdfun, zero, quantile)
{
