gbm.valuemap<-function(dbase,  # data.frame to load. Expects Lon, Lat & data columns: predicted abundances, fishing effort etc. E.g.: Abundance_Preds_All.csv from gbm.auto
                       ...
                       HRMSY = NULL, # maximum percentage of each goodcols stock which can be removed each year, as decimal e.g. 0.15 = 15%. So we need to protect the rest i.e. 1-HRMSY. Single number or vector. Same order as goodcols
...
require(mapplots) #for maps
library(mapplots)
if(alerts) options(error = function() {beep(9)})  # give warning noise if it fails #object alerts not found
data(coast,package="mapplots")  # get Britain & Ireland coast data. I'm looking to make this global
...
if("bad" %in% plotthis){
  ...
  if(BnW){
    ...
    gbm.map(x = dbase[,loncolno],  # run gbm.map function with generated parameters
            ...
            mapback = "white",
            heatcolours = grey.colors(8, start=1, end=0)) # passes the lgenedtitleV set by user in gbm.valuemap call to legendtitle in gbm.map
    dev.off()
  }} # close badplot & BnW
...
if(BnW){
  ...
  gbm.map(x = dbase[,loncolno],
          ...
          landcol = grey.colors(1, start=0.8, end=0.8), #light grey. 0=black 1=white
          mapback = "white",
          heatcolours = grey.colors(8, start=1, end=0)) # passes the legendtitleV set by user in gbm.valuemap call to legendtitle in gbm.map
  dev.off()
}} # close goodplot & BnW
...
if("both" %in% plotthis){
  ...
  if(BnW){
    ...
    gbm.map(x = dbase[,loncolno],
            ...
            landcol = grey.colors(1, start=0.8, end=0.8), #light grey. 0=black 1=white
            mapback = "white",
            heatcolours = grey.colors(3, start=1, end=0))  # optional terms: byx byy mapmain heatcol shape mapback landcol legendtitle lejback legendloc grdfun zero quantile
    dev.off()
  }} # close bothplot & BnW
...
# 1: largest to smallest bothdata value for that species, i.e. highest cpue lowest e combo.
# 2: largest to smallest gooddata value for that species, i.e. highest biomass.
# 3: smallest to largest baddata value (universal) THEN largest to smallest gooddata value for that species, i.e. lowest effort THEN highest biomass
# 4: largest to smallest conserve value (universal), i.e. highest conservation value (includes all species)
...
for(o in 1:length(maploopnames)){ # start o loop through combination, goodcols, badcols, conservecol
...
if("close" %in% plotthis){  # min area for MRMSY, starting with best fish cells
  ...
  CPUEMSY <- (sum(dbase[,goodcols[j]]) * HRMSY[j]) # HSMSY% * total biomass for J'th species = biomass to protect i.e. 'sum-to threshold'
  dbase <- eval(parse(text = maploopcodes[o])) #sort according to O'th maploopcode
   for(k in 1:nrow(dbase)){ # run the loop for every row, if required
      print(paste("Run ",((o-1)*length(maploopcodes))+j," of ",length(goodcols)*length(maploopcodes),"; ",n,", ",round((sum(dbase[n:nrow(dbase),goodcols[j]])/CPUEMSY)*100,3),"%",sep=""))  # progress printer
          if(sum(dbase[n:nrow(dbase),goodcols[j]]) < CPUEMSY) {n = n-1} else { #if sum of rows to this point < CPUEMSY add another row and sum again, ELSE:
...
print(paste("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX     Overlay Map ",((o-1)*length(maploopcodes))+j," of ",length(goodcols)*length(maploopcodes),"    XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",sep=""))
              ...
              y = dbase[,latcolno]
              z = dbase[,ncol(dbase)] #last column, cbound @ L128, sort column, zeroes populated with 1s.
            ...
            ca_x <- dbase[,loncolno] #set parameters for closed area map
            ca_y <- dbase[,latcolno]
            ca_z <- dbase[,ncol(dbase)] #last column, cbound @ L128, sort column, zeroes populated with 1s.
            ...
            if(BnW){
              png(filename = paste("./ClosedValueMap_",maploopnames[o],"_",goodname[j],"_BnW.png",sep=""), # map species j's bothdata with black closed areas overlaid
                  width = 4*1920, height = 4*1920, units = "px", pointsize = 4*48, bg = "white", res = NA, family = "", type = "cairo-png")
              par(mar=c(3.2,3,1.3,0), las=1, mgp=c(2.1,0.5,0),xpd=FALSE)
              ...
              colournumber = 8   #number of colours to spread heatcol over, default:8
              shape = coast   #basemap shape to draw, from draw.shape in mapplots. Defaults: 'coast': UK & Ire
              landcol = grey.colors(1, start=0.8, end=0.8) #colour for 'null' area of map, if appropriate, from draw.shape in mapplots. Was "darkgreen" changed to light grey
              mapback = "white" #basemap background
              legendloc = "bottomright" #location on map of legend box, from legend.grid in mapplots
              legendtitle = "0 - 2" #the metric of abundance, e.g. CPUE for fisheries, from legend.grid in mapplots
              lejback = "white"  #backgroud colour of legend, from legend.grid in mapplots
              zero = TRUE # allow 0 category in breaks.grid & thus legend? Default TRUE
...
if("close" %in% plotthis){ #re-open a "close" optional section now j loop finished, to process dbase
  SortCol0 <- ncol(dbase)-length(goodcols)  # sort largest to smallest species 1 value, then 2, 3 4
...
# then create a new df with a column of zeroes called Zeroes, length = nrow(dbase)
...
# bind to dbase, removing zeroes
dbase <- cbind(dbase,MPAgrow[,2:ncol(MPAgrow)])
####how to do this algorithmically so I don't have to know how many species there are?####
# loop L216-218 (assign / MPAgrow / colnames) for n=length(goodcols) somehow?
...
print(paste("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX Cumulative Closed Area Map ",((o-1)*length(maploopcodes))+l," of ",length(goodcols)*length(maploopcodes)," XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",sep=""))
  ...
  } # end of l loop (column numbers of growing MPA in dbase)
...
gbm.map(x = dbase[,loncolno],
        y = dbase[,latcolno],
        z = dbase[,ncol(dbase)],
        mapmain = "Per Species Closed Area: ",
...
if(BnW){
  ...
  gbm.map(x = dbase[,loncolno],
          ...
          mapmain = "Per Species Closed Area: ",
          species = maploopnames[o],
          heatcolours = grey.colors(8, start=0.7, end=0), #black then rainbow colours. Total n is length(goodcols): blank, black, then goodcols-1 other colours
          colournumber = length(goodcols)+1, # Colours are set as the breaks, but painted from the midpoints
          shape = coast,
          landcol = grey.colors(1, start=0.8, end=0.8), #light grey. 0=black 1=white
          mapback = "white",
...
####save csvs####
print(paste("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX         Saving CSV        XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",sep=""))
