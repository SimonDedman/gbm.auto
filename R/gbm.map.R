...
gbm.map <- function(x,        #vector of longitudes, from make.grid in mapplots; x. ##Order by this (descending) SECOND
                    ...
                    grdfun = c(mean, sum, prod, min, max, sd, se, var), #make.grid operand for >=2 values per cell. Default:mean, other options: sum prod min max sd se var.
...
# png(...); par(...); gbm.map(...); dev.off()
{
  ...
  grdfun <- match.arg(grdfun) # populate object from function argument in proper way
