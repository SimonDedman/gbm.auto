...
# read.csv & specify object name. Defaults to NULL (won't predict to grids)
  samples = mysamples,  # explanatory & response variables to predict from.
...
# explanatory variables. Default 1*white: white bars black borders. '1*' repeats
  ...
  savegbm = TRUE,       # save gbm objects? Open w/ load("Bin_Best_Model")
...
# for error "contrasts can be applied only to factors with 2 or more levels"
  ...
  binonly = FALSE,      # run only binomial sections of code?
...
# that. Generates line, dot & bar plots, and outputs these and the predictions
...
if (!binonly) { # if processing Gaussian data run these lines
...
# combo in the loop, & the name of that BRT combo, per species (m is reset at
# the end of each species loop so m=1 is the first loop per species). Then if
# subsequent combos have higher values (more correlation), it replaces the value
# and name of combo it came from

Bin_Best_Score <- 0 # create blanks for best results
Bin_Best_Model <- 0
...
n <- n + 1}   # Add to print counter & close ZI option
...
if (!binonly) {Report[1:6,((m*5) + 2)] <- c(paste("trees: ",get(paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$n.trees, sep = ""),
...
Report[1,(m*5) + 3] <- paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = "")} # close binonly
# different colnames depending on ZI state
if (!binonly) {if (ZI) {colnames(Report)[((m*5) - 1):((m*5) + 3)] <- c(paste("Parameter Combo ",m, sep = ""),
        ...
        colnames(Report)[((m*5) - 1):((m*5) + 3)] <- c(paste("Parameter Combo ",m, sep = ""),
                                                       ...
                                                       paste("Gaussian BRT ",m," stats", sep = ""),
                                                       paste("Gaussian BRT ",m," name", sep = ""))
}} #close binonly & ifelse

if (binonly) {if (ZI) {colnames(Report)[((m*5) - 1):((m*5) + 1)] <- c(paste("Parameter Combo ",m, sep = ""),
                                                                       paste("Binary BRT ",m," stats", sep = ""),
                                                                       paste("Binary BRT ",m," name", sep = ""))
} else {
  colnames(Report)[((m*5) - 1):((m*5) + 1)] <- c(paste("Parameter Combo ",m, sep = ""),
                                                 paste("No Binary BRT"),
                                                 paste("No Binary BRT"))
}} #close binonly & ifelse
...
if (alerts) beep(2) # progress printer, right aligned for visibility & close ZI
print(paste("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX    Simplified Bin model    XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", sep = ""))}

# Same for Gaus
  if (!binonly) {Gaus_Best_Simp_Check <- gbm.simplify(get(Gaus_Best_Model))
...
# Same for Gaus:
  if (!binonly) {if (min(Gaus_Best_Simp_Check$deviance.summary$mean) < 0)
...
if (!binonly) {png(filename = paste("./",names(samples[i]),"/Gaus_Best_line.png", sep = ""),
    width = 4*480, height = 4*480, units = "px", pointsize = 4*12, bg = "white", res = NA, family = "", type = pngtype)
...
if (!binonly) {for (p in 1:length(get(Gaus_Best_Model)$contributions$var)) {
...
if (!binonly) {png(filename = paste("./",names(samples[i]),"/Gaus_Best_dot.png", sep = ""),
    width = 4*480, height = 4*480, units = "px", pointsize = 4*12, bg = "white", res = NA, family = "", type = pngtype)
...
if (!binonly) {Gaus_Bars <- summary(get(Gaus_Best_Model),
...
if (!binonly) {png(filename = paste("./",names(samples[i]),"/Gaus_Bars.png", sep = ""),
    width = 4*480, height = 4*480, units = "px", pointsize = 4*12, bg = "white", res = NA, family = "",
    type = pngtype)
par(mar = c(2.5,0.3,0,0.5), fig = c(0,1,0,1), cex.lab = 0.8,mgp = c(1.5,0.5,0), cex = 1.3)
...
if (!binonly) if (varint) find.int_Gaus <- gbm.interactions(get(Gaus_Best_Model))
if (alerts) beep(2) # progress printer, right aligned for visibility
...
if (!binonly) {gbm.predict.grids(get(Gaus_Best_Model), grids, want.grids = F, sp.name = "Gaus_Preds")
...
#if ZI=TRUE, unlog gaus & multiply by bin. Else just use gaus preds. Close binonly
grids$PredAbund <- grids$Gaus_Preds_Unlog * grids$Bin_Preds} else {grids$PredAbund <- Gaus_Preds}}
if (binonly) grids$PredAbund <- Bin_Preds # binonly predabund = bin probability
predabund <- which(colnames(grids) == "PredAbund") # predabund col no for writecsv
...
if (ZI) {save(Bin_Best_Model_Object,file = paste("./",names(samples[i]),"/Bin_Best_Model", sep = ""))}

if (!binonly) { Gaus_Best_Model_Object <- get(Gaus_Best_Model)
...
if (!binonly) Report[1:2,(reportcolno - 12)] <- c(paste("Model combo: ", Gaus_Best_Model, sep = ""), paste("Model CV score: ", Gaus_Best_Score, sep = ""))
...
if (!binonly) if (simp) {Report[1:dim(subset(Gaus_Best_Simp_Check$final.drops,order > 0))[1], (reportcolno - 9)] <- as.character(subset(Gaus_Best_Simp_Check$final.drops ,order > 0)$preds)}
if (!binonly) {if (simp) {Report[1:(length(Gaus_Best_Simp_Check$final.drops$preds) - dim(subset(Gaus_Best_Simp_Check$final.drops, order > 0))[1]), (reportcolno - 8)] <-
...
if (ZI) Report[1:6,(reportcolno - 7)] <-   c(paste("trees: ", get(paste("Bin_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$n.trees, sep = ""),
                                   ...
                                   paste("CV Mean Correlation: ",get(paste("Bin_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$cv.statistics$correlation.mean, sep = ""),
                                   paste("CV Correlation SE: ",get(paste("Bin_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$cv.statistics$correlation.se, sep = ""))
if (!binonly) {Report[1:6,(reportcolno - 6)] <- c(paste("trees: ", get(paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$n.trees, sep = ""),
                                 paste("Training Data Correlation: ",get(paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$self.statistics$correlation[[1]], sep = ""),
                                 paste("CV Mean Deviance: ",get(paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$cv.statistics$deviance.mean, sep = ""),
                                 paste("CV Deviance SE: ",get(paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$cv.statistics$deviance.se, sep = ""),
                                 paste("CV Mean Correlation: ",get(paste("Gaus_BRT",".tc",j,".lr",k*100,".bf",l, sep = ""))$cv.statistics$correlation.mean, sep = ""),
...
if (!binonly) Report[1:(length(Gaus_Bars[,1])),(reportcolno - 3)] <- as.character(Gaus_Bars$var)
if (!binonly) Report[1:(length(Gaus_Bars[,2])),(reportcolno - 2)] <- as.character(Gaus_Bars$rel.inf)
...
if (!binonly) {if (varint) Report[1:2,(reportcolno)] <- c(paste(find.int_Gaus$rank.list$var1.names[1]," and ",find.int_Gaus$rank.list$var2.names[1],". Size: ",find.int_Gaus$rank.list$int.size[1], sep = ""),
...
# builds doesn't plot surface. If built, plotted by map maker.
  ...
  if (!binonly) {pos_samples <- subset(samples, brv > 0)
  ...
  if (binonly) write.csv(rsbdf_bin, row.names = FALSE, file = paste("./", names(samples[i]), "/RSB.csv", sep = ""))
...
if (!binonly) {png(filename = paste("./",names(samples[i]),"/RSB_Map_Gaus_",names(samples[i]),".png", sep = ""),
        width = 4*1920, height = 4*1920, units = "px", pointsize = 4*48, bg = "white", res = NA, family = "", type = pngtype)
    par(mar = c(3.2,3,1.3,0), las = 1, mgp = c(2.1,0.5,0),xpd = FALSE)
    gbm.map(x = grids[,gridslon],
            y = grids[,gridslat],
...
if (!binonly) {png(filename = paste("./",names(samples[i]),"/RSB_Map_BnW_Gaus_",names(samples[i]),".png", sep = ""),
        width = 4*1920, height = 4*1920, units = "px", pointsize = 4*48, bg = "white", res = NA, family = "", type = pngtype)
    par(mar = c(3.2,3,1.3,0), las = 1, mgp = c(2.1,0.5,0),xpd = FALSE)
    gbm.map(x = grids[,gridslon],
            y = grids[,gridslat],
